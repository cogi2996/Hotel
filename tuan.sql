...
AS
	SELECT KhachHang.MaKH, KhachHang.TenKH, KhachHang.NgaySinh, 
		   ...
		   DatPhong.SoPhong,DatPhong.LoaiPhong, DatPhong.CheckIn
	FROM KhachHang
...
go
 ...
 -- 9. VIEW danh sách sử dụng dịch vụ của một khách hàng
...
exec proc_DanhSachSuDungDichVu 21
...
-- 6.FUNCTION tính ổng tiền hoá đơn
...
BEGIN
    BEGIN TRY
        ...
        IF @TenKH IS NULL OR @NgaySinh IS NULL OR @CCCD IS NULL OR @SDT IS NULL OR @LoaiKH IS NULL
        BEGIN
            RAISERROR ('Tên khách hàng, ngày sinh, CCCD, SDT hoặc loại khách hàng không được để trống. Vui lòng kiểm tra lại!', 16, 1);
            RETURN;
        END
        IF @LoaiKH NOT IN ('V', 'T')
        BEGIN
            RAISERROR ('Loại khách hàng phải là V hoặc T', 16, 1);
            RETURN;
        END
        IF LEN(@CCCD) != 12
        BEGIN
...
BEGIN
    BEGIN TRY
        ...
        BEGIN
            ...
            RETURN;
        END
        IF EXISTS (SELECT * FROM KhachHang WHERE CCCD = @CCCD AND MaKH != @MaKH)
        BEGIN
            RAISERROR ('CCCD đã tồn tại!', 16, 1);
            RETURN;
        END
